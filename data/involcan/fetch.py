...
def find_seismic_files(sftp, remote_path, writer, server, existing_files):
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_directory, root, files, writer, server, existing_files) 
                   for root, dirs, files in sftp_walk(sftp, remote_path)]
        for future in as_completed(futures):
            future.result()

def process_directory(root, files, writer, server, existing_files):
    for file in files:
        if is_seismic(file):
            ...
            if file_name not in existing_files:
                ...
                if network == "C7" and (sensor in lp_stations) and re.match(r'^HH.', channel):
                    writer.writerow([network, sensor, channel, starttime, endtime, server, root.replace('\\', '/'), file_name])
    ...
    try:
        ...
        with open(log_path, mode=write_mode, newline='') as file:
            ...
            if mode == 'overwrite' or write_mode == 'w':
                writer.writerow(['network', 'sensor', 'channel', 'year', 'month', 'day', 'hour', 'minute', 'second', 'server', 'path', 'filename'])
