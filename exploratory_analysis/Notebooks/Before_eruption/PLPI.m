...
index = find(strcmp({textHandles.String}, '18-Sep-2021 22:00:00'));
set(textHandles(index), 'Position', [1.55e20, 0.3e9]);
index = find(strcmp({textHandles.String}, '19-Sep-2021 00:00:00'));
set(textHandles(index), 'Position', [2.05e9, -1.45e9]);
index = find(strcmp({textHandles.String}, '14-Sep-2021 07:00:00'));
set(textHandles(index), 'Position', [2.11e9, -.65e9]);
...
set(textHandles(index), 'Position', [1.9e9, 2.5e9]);

saveas(gcf, folder_path + 'PLPI_scores_2PCs', 'epsc');
saveas(gcf, folder_path + 'PLPI_scores_2PCs', 'png');
%% Average magnitude - 2PCs
scores(model, 'ObsLabel',obs_label, 'ObsClass', avg_mag, 'opt', '00100','BlurIndex', 0.050);
colorbar()
title("PLPI - Average magnitude of events")
%%
f = gcf;
f.Position = [100 100 640 500];
textHandles = findobj(f, 'Type', 'Text');
...
saveas(gcf, folder_path + 'PLPI_scores_avg_mag_2PCs', 'epsc');
saveas(gcf, folder_path + 'PLPI_scores_avg_mag_2PCs', 'png');
...
legend()
%%
saveas(gcf, folder_path + 'PLPI_loadings_2PCs', 'epsc');
saveas(gcf, folder_path + 'PLPI_loadings_2PCs', 'png');

%% Scores - Maximum magnitude - 1PC
figure
tit = "PLPI - Maximum magnitude of events";
...
xticks(50:75:num_obs);
xticklabels(obs_label(50:75:num_obs));
grid on;

f = gcf;
f.Position = [100 100 640 500];

saveas(gcf, folder_path + 'PLPI_scores_1PC', 'epsc');
saveas(gcf, folder_path + 'PLPI_scores_1PC', 'png');
...
saveas(gcf, folder_path + 'PLPI_scores_avg_mag_1PC', 'epsc');
saveas(gcf, folder_path + 'PLPI_scores_avg_mag_1PC', 'png');
%%
%% Loadings 1PC
...
ylim([0,0.08])
%%
saveas(gcf, folder_path + 'PLPI_loadings_1PC', 'epsc');
...
saveas(gcf, folder_path + 'PLPI_var_ckf_no_eq', 'epsc');
saveas(gcf, folder_path + 'PLPI_var_ckf_no_eq', 'png');
...
saveas(gcf, folder_path + 'PLPI_loadings_no_eq', 'epsc');
saveas(gcf, folder_path + 'PLPI_loadings_no_eq', 'png');
...
saveas(gcf, folder_path + 'PLPI_scores_no_eq', 'epsc');
saveas(gcf, folder_path + 'PLPI_scores_no_eq', 'png');
