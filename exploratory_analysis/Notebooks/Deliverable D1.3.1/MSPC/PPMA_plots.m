...
saveas(gcf, folder_path + 'PPMA_scores_1PC_avg_mag', 'png');
...
saveas(gcf, folder_path + 'PPMA_scores_1PC_before_after', 'png');
%% Loadings
...
f=gcf;
grid on;
saveas(f, folder_path + ['PPMA_var_ckf','_train',train_days,'_test',test_days], 'png');
...
for i = 1:length(figures)
    ...
    % close all
    ['PPMA_',yl.String,'_train',train_days,'_test',test_days]
    saveas(f, folder_path + ['PPMA_',yl.String,'_train',train_days,'_test',test_days], 'png');
end
%%
...
eruption_start = find(obs_label == "19-Sep-2021 15:00:00")+2;
forecast_start = find(obs_label == "19-Sep-2021 04:00:00")+2;
...
for i = 1:length(figures)
    ...
    id_eruption = find(test_ids == eruption_start);
    id_forecast = find(test_ids == forecast_start);
    ax = f.Children;
    barHandle = findobj(ax, 'Type', 'Bar');
    barHandle(1).FaceColor = 'flat';
    % barHandle(1).CData(:, :) = repmat([ 0.9020, 0.6240, 0], length(barHandle(1).CData), 1); 
    barHandle(1).CData(:, :) = repmat([0.902,0.624,0], length(barHandle(1).CData), 1); 
    if ~isempty(id_eruption)
        barHandle(1).CData(id_eruption, :) = [0.337,0.706,0.914]; 
    end
    if ~isempty(id_forecast)
        barHandle(1).CData(id_forecast, :) = [0,0.620,0.451]; 
    end
    % barHandle(1).CData(5, :) = [0,0.620,0.451]; 
    ...
    texto1 = {'Eruption start at 14:10'};
    texto2 = {'Forecast at 4:00'};
    if i == 1
    text(89 , 9e2, texto1, 'Color', [0.337,0.706,0.914], 'FontSize', 20, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    text(89 , 5e1, texto2, 'Color', [0,0.620,0.451], 'FontSize', 20, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    end

    if i == 2
    text(89 , 2e19, texto1, 'Color', [0.337,0.706,0.914], 'FontSize', 20, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    text(89 , 2e17, texto2, 'Color', [0,0.620,0.451], 'FontSize', 20, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    end

    saveas(f, folder_path + ['forecast_demo'], 'png');
end
...
function txt = myCallback(~, event_obj)
    pos = get(event_obj, 'Position');
