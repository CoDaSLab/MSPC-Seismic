...
from preprocessing.fft_rt import calculate_fft_rt, delete_fft
...
def monitoring(config_path = 'config.json'):
    ...
    latest_pulls_log_path = config["latest_pulls_log_path"]
    ...
    window_shift = window_size if config["window_shift"] is None else config["window_shift"]
    detrend = config["detrend"]
    feature_types = config["feature_types"]
    fft_points = config["fft_points"]
    merge_method = config["merge_method"]
    merge_fill_value = config["merge_fill_value"]
    pad_fill_value = config["pad_fill_value"]
    cpus = config["cpus"]
    verbose = config["verbose"]
    days_before_delete = config["days_before_delete"]
    ...
    for station in avail_stations:
        ...
        # Perform MSPC for all NOCs associated with the sensor
        test_data = np.hstack([features[key] for key in feature_types])  # one or more of [ffts, deltas_ffts, deltas_deltas_ffts]
        mspc(nocs[station], test_data, starttime, endtime, window_size, window_shift, plot=True,
             anomaly_log_path=anomaly_log_path, noc_log_path=noc_log_path,
...
def get_available_stations(csv_path = "data/involcan/metadata/latest_pulls.csv", tolerance = 5):
    """
    Returns a sorted list of unique sensor names whose latest download time
    is within the last `minutes` minutes from now (UTC).
    
    Parameters
    ----------
        csv_path (str): Path to the latest pulls CSV file.
        tolerance (int or timedelta): Tolerance for considering a sensor 
            available. If int, minutes are assumed (default: 5).
        
    Returns
    -------
        list: List of available sensor names.
    """
