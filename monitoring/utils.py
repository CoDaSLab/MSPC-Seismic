...
def create_noc(station, starttime, endtime, config, attempt_find=False):
    """
    ...
    if attempt_find:  # Try to find existing features
        features = find_features(features_path, station, starttime, endtime, feature_types, 
                                additional_matches=config["features"], verbose=verbose)
        if features:
    ...
    new_noc = NOC(new_name, new_features, features['obs_labels'], network, station, type='dynamic',
                    preprocessing = 1, n_components = 'auto', quantile_threshold = 0.99, csv_path=noc_log_path)
