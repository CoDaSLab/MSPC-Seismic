...
def change_config(config, config_path):
    ...
    with st.form(key):
        ...
        with col1:
            new_config["update_frequency"] = st.number_input("Update frequency (in minutes)", value=config["update_frequency"], min_value=0)
            new_config["delay"] = st.number_input("Delay (in minutes)", value=config["delay"], min_value=0)
            new_config["data_path"] = st.text_input("Raw signal data directory", value=config["data_path"])
            new_config["features_path"] = st.text_input("Features directory", value=config["features_path"])
            new_config["nocs_path"] = st.text_input("NOCs directory", value=config["nocs_path"])
        with col2:
            new_config["save_plots"] = st.checkbox("Save plots?", value=config["save_plots"],
                                               help = "If checked, all plots generated will be saved in the specified path.")
            new_config["plots_path"] = st.text_input("Plots directory", value=config["plots_path"])
            new_config["latest_pulls_log_path"] = st.text_input("Latest pulls log file", value=config["latest_pulls_log_path"])
            new_config["noc_log_path"] = st.text_input("NOC log file", value=config["noc_log_path"])
            new_config["anomaly_log_path"] = st.text_input("Anomaly log file", value=config["anomaly_log_path"])
        
        st.header("Stations and channels")
        new_config["network"] = st.text_input("Network", value=config["network"], disabled=True)
        new_config["stations"], new_config["channels"] = forms.multi_select_station(key, station_list, default_stations=None, 
                                                                                    default_channels=['HHE', 'HHN', 'HHZ'])
        
        st.header("FFT parameters")
        col1, col2 = st.columns(2)
        with col1:
            new_config["window_size"] = st.number_input("Window size (in seconds)", value=config["window_size"], min_value=1)
            new_config["window_shift"] = st.number_input("Window shift (in seconds)", value=config["window_shift"] if config["window_shift"] else 0,
            ...
            new_config["windowing"] = forms.select_windowing("windowing")
        with col2:
            new_config["detrend"] = forms.select_detrend("Detrend method")
            feature_options = ["ffts", "deltas_ffts", "deltas_deltas_ffts"]
            new_config["feature_types"] = st.multiselect("Select feature types", options=feature_options, default=feature_options[0],
                                            help="Available feature types are FFT coefficients and derivatives.")
            fft_auto = st.checkbox("Set FFT points automatically", value=True)
            new_config["fft_points"] = forms.select_FFT_points(key, new_config["window_size"])
            if fft_auto:
                new_config["fft_points"] = 'auto'
        
        with st.expander("Advanced settings"):
            col1, col2 = st.columns(2)
            with col1:
                new_config["merge_method"] = st.number_input("Merge method", value=config["merge_method"], min_value=0, max_value=2, step=1,
                ...
                new_config["merge_fill_value"] = st.text_input("Merge fill value", value=config["merge_fill_value"],
                                                        help="Value to fill gaps in the middle of a signal when merging traces. Follows the ObsPy notation: " \
                                                        "https://docs.obspy.org/packages/autogen/obspy.core.trace.Trace.html#obspy.core.trace.Trace.__add__")
                new_config["pad_fill_value"] = st.number_input("Pad fill value", value=config["pad_fill_value"],
                                                            help="Value to fill gaps at the beginning or end of a signal.")
                new_config["cpus"] = st.number_input("Number of CPUs for feature extraction", value=config["cpus"], min_value=1)
            with col2:
                # config["verbose"] = st.checkbox("verbose", value=config["verbose"])
                new_config["num_days_before_delete"] = st.number_input("Time before deletion (in days)", value=config["num_days_before_delete"], min_value=1,
                                                                help="Saved features, NOCs and graphs will be deleted after the specified number of days.")
                new_config["num_hours_plot"] = st.number_input("Time range of the saved plots (in hours)", value=config["num_hours_plot"], min_value=1)
                new_config["num_days_noc_update_frequency"] = st.number_input("NOC update frequency (in days)", value=config["num_days_noc_update_frequency"], min_value=1)
                new_config["num_days_noc_length"] = st.number_input("Time range of NOC data (in days)", value=config["num_days_noc_length"], min_value=1)
